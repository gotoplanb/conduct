...
async def export_sft(
    ...
    whose average quality score is >= min_score."""
    rows = await iter_sft(
        session, task_type=task_type, min_score=min_score, via=via,
...
async def export_preferences(
    ...
    pointwise/panel score differentials (min_gap) on the same input."""
    rows = await iter_preferences(
        session, task_type=task_type, method=method, min_gap=min_gap,
