    ...
    try:
        async with SessionLocal() as session:
            for spec in seed_clients:
                name = spec["name"]
                existing = await session.scalar(select(ClientApp).where(ClientApp.name == name))
                if existing is not None:
                    skipped_clients.append(name)
                    continue
                ...
                created_clients.append((name, raw_key))

            for spec in seed_routing:
                tt = spec["task_type"]
                existing_rule = await session.scalar(
                    select(RoutingRule).where(RoutingRule.task_type == tt)
                )
                if existing_rule is not None:
                    skipped_rules.append(tt)
                    continue
